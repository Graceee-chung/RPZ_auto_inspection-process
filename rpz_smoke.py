...
def process_folder_smoke(folder_path, folder_name, base_folder, outlook):
    ...
    match_docnum = re.search(r'衛授國字第\d+號', text.replace(" ", ""))
    if not match_docnum:
    ...
    for file in ods_files:
        ...
        for idx, row in df.iterrows():
            ...
            email = str(row['承辦人email']).strip().lower() if '承辦人email' in row else "bb89106049@hpa.gov.tw"
            law_refer = str(row['法律依據']).strip().lower() if '法律依據' in row else "菸害防制法"
            unit = str(row['聲請單位']).strip().lower() if '聲請單位' in row else "衛生福利部"
            channel = str(row['申訴管道']).strip().lower() if '申訴管道' in row else "https://www.hpa.gov.tw/Home/Index.aspx"
